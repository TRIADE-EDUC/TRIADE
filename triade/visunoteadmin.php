...
if (($_SESSION["membre"] == "menuprof") && (isset($_SESSION["profP"]))) {
        print "<form method='post' action='profp2.php' target='_parent' >";
        print "<div><script language='JavaScript'>buttonMagicSubmitAtt('Retour menu principal','create','');</script></div>";
        print "<input type='hidden' name='saisie_classe' value='".$cgrp1."' />";
	print "<input type='hidden' name='saisie_pers' value='".$_GET['saisie_pers']."' /></form><br><br>";
}
...
//$worksheet1->set_selection('A0');
	...
	for($i=0;$i<count($mat);$i++){
		...
		for($t=0;$t<count($data_note);$t++){
			...
			if ($sujetaff == 0) {
					$pdf->SetFont('Arial','',9);
					$pdf->TextWithRotation($xsujet,$ysujet,utf8_decode(trunchaine($sujetpdf,13)),45,-45);
					$xsujet1=$xsujet+4;
			...
			if ($okmoy == 1) {
				...
				$pdf->SetFillColor(220);
			        ...
			        $pdf->SetTextColor($rgb1,$rgb2,$rgb3);
				$pdf->MultiCell(12,5,strtolower($moyenneleveaff),1,'',1);
