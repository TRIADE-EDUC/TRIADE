...
if ( (!empty($fichier)) && (($type == "application/octet-stream" ) || ($type == "application/vnd.ms-excel" ))) {
	...
	for ($i = 1; $i <= $data->sheets[0]['numRows']; $i++) {
		...
		foreach ($tab11 as $clef => $b ) {
			if (strlen(trim($clef))) {
		    	?>
				<td><input type=text name='saisie_ref_<?php print $i ?>' value='<?php print $clef?>' size=20 onfocus='this.blur()'></td>
				<td><select name="saisie_classe_<?php print $i ?>">
			<?php
...
<input type=hidden  value='<?php print $i ?>' name='nbi'  >
