...
class ArWordTag
{
    protected static $_particle_pre_nouns = array('Úä', 'Ýí', 'ãÐ', 'ãäÐ', 'ãä', 'Çáì', 'Úáì', 'ÍÊì', 'ÇáÇ', 'ÛíÑ', 'Óæì', 'ÎáÇ', 'ÚÏÇ', 'ÍÇÔÇ', 'áíÓ');
    protected static $_normalizeAlef = array('Ã','Å','Â');
    protected static $_normalizeDiacritics = array('ó','ð','õ','ñ','ö','ò','ú','ø');
    ...
    public static function isNoun($word, $word_befor, $inputCharset = null, $main = null)
    {
        ...
        $word = str_replace(self::$_normalizeAlef, 'Ç', $word);
        $word_befor = str_replace(self::$_normalizeAlef, 'Ç', $word_befor);
        ...
        if (substr($word, 0, 2) == 'Çá' && $wordLen >= 5) {
            return true;
        }
        
        if (in_array(substr($word, -1), array('ì','Á','É')) && $wordLen >= 4) {
            return true;
        }

        if (substr($word, -2) == 'ÇÊ' && $wordLen >= 5) {
            return true;
        }

        if (preg_match('/^ã\S{3}$/', $word) || preg_match('/^ã\S{2}Ç\S$/', $word) || preg_match('/^ã\S{3}É$/', $word) || preg_match('/^\S{2}Ç\S$/', $word) || preg_match('/^\SÇ\Sæ\S$/', $word) || preg_match('/^\S{2}æ\S$/', $word) || preg_match('/^\S{2}í\S$/', $word) || preg_match('/^ã\S{2}æ\S$/', $word) || preg_match('/^ã\S{2}í\S$/', $word) || preg_match('/^\S{3}É$/', $word) || preg_match('/^\S{2}Ç\SÉ$/', $word) || preg_match('/^\SÇ\S{2}É$/', $word) || preg_match('/^\SÇ\Sæ\SÉ$/', $word) || preg_match('/^Ç\S{2}æ\SÉ$/', $word) || preg_match('/^Ç\S{2}í\S$/', $word) || preg_match('/^Ç\S{3}$/', $word) || preg_match('/^\S{3}ì$/', $word) || preg_match('/^\S{3}ÇÁ$/', $word) || preg_match('/^\S{3}Çä$/', $word) || preg_match('/^ã\SÇ\S{2}$/', $word) || preg_match('/^ãä\S{3}$/', $word) || preg_match('/^ãÊ\S{3}$/', $word) || preg_match('/^ãÓÊ\S{3}$/', $word) || preg_match('/^ã\SÊ\S{2}$/', $word) || preg_match('/^ãÊ\SÇ\S{2}$/', $word) || preg_match('/^\SÇ\S{2}$/', $word)) {
            return true;
        }
