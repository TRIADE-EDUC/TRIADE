...
class ArQuery
{
    ...
    protected function _allWordForms($word) 
    {
        ...
        $postfix1 = array('ßã', 'ßä', 'äÇ', 'åÇ', 'åã', 'åä');
        $postfix2 = array('íä', 'æä', 'Çä', 'ÇÊ', 'æÇ');
        
        $len = strlen($word);

        if (substr($word, 0, 2) == 'Çá') {
            $word = substr($word, 2);
        }
        ...
        if (($len >= 4 && ($last1 == 'É' || $last1 == 'å' || $last1 == 'Ê')) || ($len >= 5 && $last2 == 'ÇÊ')) {
            $wordForms[] = $str1;
            $wordForms[] = $str1 . 'É';
            $wordForms[] = $str1 . 'å';
            $wordForms[] = $str1 . 'Ê';
