 ...
 * trigonometry. A is a given location, K is the Kaâba, and N is the North Pole. 
 ...
 * direction AN to the direction AK towards the Kaâba, and so q is the qibla 
 * bearing to be computed. Let Î¦ and Î» be the latitude and longitude of A, and Î¦K 
 * and Î»K be the latitude and longitude ofK (the Kaâba). If all angles and arc 
 * lengths are measured in degrees, then it is seen that the arcs AN and KN are 
 * of measure 90 - Î¦ and 90 - Î¦K, respectively. Also, the angle ANK between the 
 * meridians of K and A equals the difference between the longitudes of A and K, 
 * that is, Î»K - Î», no matter what the prime meridian is. Here we are given two 
...
class Salat
{
    ...
    // ÙØ±Ù Ø§ÙØªÙÙÙØª Ø§ÙØ¹Ø§ÙÙÙ
    protected $_zone = 2;
    
    // Ø®Ø· Ø§ÙØ·ÙÙ Ø§ÙØ¬ØºØ±Ø§ÙÙ ÙÙÙÙØ§Ù
    protected $_long = 37.15861;
    
    // Ø®Ø· Ø§ÙØ¹Ø±Ø¶ Ø§ÙØ¬ØºØ±Ø§ÙÙ
    protected $_lat = 36.20278;
    
    // Ø²Ø§ÙÙØ© Ø§ÙØ´Ø±ÙÙ ÙØ§ÙØºØ±ÙØ¨
    ...
    public function getPrayTime()
    {
        ...
        if ($this->_school == 'Shafi') {
            // ÙØ­Ø³Ø¨ Ø¥Ø±ØªÙØ§Ø¹ Ø§ÙØ´ÙØ³ ÙÙÙØª ØµÙØ§Ø© Ø§ÙØ¹ØµØ± Ø­Ø³Ø¨ Ø§ÙÙØ°ÙØ¨ Ø§ÙØ´Ø§ÙØ¹Ù
            $T = atan(1 + tan(abs($this->_lat - $Dec) * pi() / 180)) * 180 / pi();
            
            // Ø«Ù ÙØ­Ø³Ø¨ ÙÙØ³ Ø§ÙØ¯Ø§Ø¦Ø± Ø£Ù Ø§ÙÙÙØª Ø§ÙÙØªØ¨ÙÙ ÙÙ ÙÙØª Ø§ÙØ¸ÙØ± Ø­ØªÙ ØµÙØ§Ø© Ø§ÙØ¹ØµØ± Ø­Ø³Ø¨ Ø§ÙÙØ°ÙØ¨ Ø§ÙØ´Ø§ÙØ¹Ù
            $V = acos((sin((90 - $T) * pi() / 180) - sin($Dec * pi() / 180) * sin($this->_lat * pi() / 180)) / (cos($Dec * pi() / 180) * cos($this->_lat * pi() / 180))) * 180 / pi() / 15;
            
            // ÙÙØª ØµÙØ§Ø© Ø§ÙØ¹ØµØ± Ø­Ø³Ø¨ Ø§ÙÙØ°ÙØ¨ Ø§ÙØ´Ø§ÙØ¹Ù
            $X = $local_noon + $V;
            $SAsr = $Dhuhr + $V / 24;
            ...
            // ÙØ­Ø³Ø¨ Ø¥Ø±ØªÙØ§Ø¹ Ø§ÙØ´ÙØ³ ÙÙÙØª ØµÙØ§Ø© Ø§ÙØ¹ØµØ± Ø­Ø³Ø¨ Ø§ÙÙØ°ÙØ¨ Ø§ÙØ­ÙÙÙ
            $U = atan(2 + tan(abs($this->_lat - $Dec) * pi() / 180)) * 180 / pi();
            
            // Ø«Ù ÙØ­Ø³Ø¨ ÙÙØ³ Ø§ÙØ¯Ø§Ø¦Ø± Ø£Ù Ø§ÙÙÙØª Ø§ÙÙØªØ¨ÙÙ ÙÙ ÙÙØª Ø§ÙØ¸ÙØ± Ø­ØªÙ ØµÙØ§Ø© Ø§ÙØ¹ØµØ± Ø­Ø³Ø¨ Ø§ÙÙØ°ÙØ¨ Ø§ÙØ­ÙÙÙ
            $W = acos((sin((90 - $U) * pi() / 180) - sin($Dec * pi() / 180) * sin($this->_lat * pi() / 180)) / (cos($Dec * pi() / 180) * cos($this->_lat * pi() / 180))) * 180 / pi() / 15;
            
            // ÙÙØª ØµÙØ§Ø© Ø§ÙØ¹ØµØ± Ø­Ø³Ø¨ Ø§ÙÙØ°ÙØ¨ Ø§ÙØ­ÙÙÙ
            $Z = $local_noon + $W;
            $HAsr = $Z / 24;
        ...
        // ÙØ­Ø³Ø¨ ÙØµÙ ÙÙØ³ Ø§ÙÙÙØ§Ø±
        ...
        // ÙØ­Ø³Ø¨ ÙØ¶Ù Ø§ÙØ¯Ø§Ø¦Ø± ÙÙÙ Ø§ÙÙÙØª Ø§ÙÙØªØ¨ÙÙ ÙÙ ÙÙØª ØµÙØ§Ø© Ø§ÙØ¸ÙØ± Ø¥ÙÙ ÙÙØª Ø§ÙØ¹Ø´Ø§Ø¡
        $AG = acos((sin($this->_AG2 * pi() / 180) - sin($Dec * pi() / 180) * sin($this->_lat * pi() / 180)) / (cos($Dec * pi() / 180) * cos($this->_lat * pi() / 180))) * 180 / pi();
        
        // ÙÙØª ØµÙØ§Ø© Ø§ÙØ¹Ø´Ø§Ø¡
        $AH = $local_noon + ($AG / 15);
        $Isha = $AH / 24;
        ...
        // ÙØ­Ø³Ø¨ ÙØ¶Ù Ø¯Ø§Ø¦Ø± Ø§ÙÙØ¬Ø± ÙÙÙ Ø§ÙÙÙØª Ø§ÙÙØªØ¨ÙÙ ÙÙ ÙÙØª ØµÙØ§Ø© Ø§ÙÙØ¬Ø± Ø­ØªÙ ÙÙØª ØµÙØ§Ø© Ø§ÙØ¸ÙØ±
        $AJ = acos((sin($this->_AJ2 * pi() / 180) - sin($Dec * pi() / 180) * sin($this->_lat * pi() / 180)) / (cos($Dec * pi() / 180) * cos($this->_lat * pi() / 180))) * 180 / pi();
        
        // ÙÙØª ØµÙØ§Ø© Ø§ÙÙØ¬Ø±
        $AK = $local_noon - $AJ / 15;
        $Fajr = $AK / 24;
     ...
     * (i.e. 12Â°34'56"S formula) into usual float number in degree unit scale 
     * (i.e. -12.5822 value)
     *      
     * @param string $value Coordinate presented in degrees, minutes and seconds
     *                      (i.e. 12Â°34'56"S formula)     
     *      
