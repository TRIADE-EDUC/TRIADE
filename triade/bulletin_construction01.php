...
$suppmentionpedago=$_POST["suppmentionpedago"];
...
config_param_ajout($suppmentionpedago,"suppmentionpedago");
...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	// variable eleve
	$nomEleve=ucwords(TextNoAccent($eleveT[$j][0]));
	$prenomEleve=ucfirst(TextNoAccent($eleveT[$j][1]));
	$lv1Eleve=$eleveT[$j][2];
	$lv2Eleve=$eleveT[$j][3];
	$idEleve=$eleveT[$j][4];
	...
	if ($abssconet == "oui") {
		...
		$nbabs=$nbjoursabs * 2;
		$nbabsjustifie=count(nombre_absJustifie2($idEleve,dateFormBase($dateDebut),dateFormBase($dateFin)));
		$nbabsnonjustifie=count(nombre_absNonJustifie2($idEleve,dateFormBase($dateDebut),dateFormBase($dateFin)));

		if ($nbabsjustifie == "") $nbabsjustifie="0";
		if ($nbabsnonjustifie == "") $nbabsnonjustifie="0";
	}
	//---------------------------------//
	...
	if ($abssconet == "oui") {
		...
		if (preg_match('/agrrennes\.triade-educ/i',$_SERVER['SERVER_NAME'])) {
			$appreciationbis="($nbretard retard(s) / $nbabsjustifie absence(s) justifiée(s) / $nbabsnonjustifie absence(s) injustifiée(s) ) ";
		}else{
...
//	$pdf->WriteHTML($infoeleveclasse);
	...
	for($i=0;$i<count($ordre);$i++) {
		...
		$nomprof=TextNoAccent(recherche_personne2($ordre[$i][1]));
		...
		if ($npAfficheCoef != "oui") {
			$pdf->WriteHTML('<B>'.trunchaine(strtoupper(strtolower(TextNoAccent($matiere))),19).'</B>');
		}else{
			$pdf->WriteHTML('<B>'.trunchaine(strtoupper(strtolower(TextNoAccent($matiere))),25).'</B>');
		}
...
// --------------------------------------------------------------------------------------------------------------------------------	
		$coeffaff=recupCoeff($ordre[$i][0],$idClasse,$ordre[$i][2]);
			...
			$TT=$i+1;
						...
						$commentaireeleve.=stripslashes(cherche_com_eleve($idEleve,$ordre[$TT][0],$idClasse,$_POST["saisie_trimestre"],$idprofSuivant,$idgroupeSuivant));
                                                $TT++;
                                        }else{
                                                break;
                                        }
...
// --------------------------------------------------------------------------------------------------------------------------------	
	...
	if ($commentaireeleve == "") $commentaireeleve=stripslashes(cherche_com_eleve($idEleve,$idMatiere,$idClasse,$_POST["saisie_trimestre"],$idprof,$idgroupe));
	...
	$profAff=strtoupper(strtolower(TextNoAccent(recherche_personne2($profAff))));
	$profAff=trunchaine($profAff,30);
	$pdf->MultiCell(30,3,"$profAff",'','L',0);
...
if (MODNAMUR0 == "oui") {
	...
	// mise en place des commentaires
	$commentaireeleve=stripslashes(cherche_com_scolaire_eleve_cpe($idEleve,"-10",$idClasse,$_POST["saisie_trimestre"],""));
	$commentaireeleve=preg_replace("/\n/"," ",$commentaireeleve);
	$confPolice=confPolice($commentaireeleve);  // $confPolice[0] -> Cadre ; $confPolice[1] -> Policy
...
$commentaireprofp=stripslashes(recherche_com_profP($idEleve,$_POST["saisie_trimestre"]));
