 ...
 * $passdb object, for example the uid to look up the hash in a custom table
 * or remote database.
 *
 * @param $password
 *   A plain-text password
 ...
 *   TRUE or FALSE.
 */
function user_check_password($password, $passdb) {
  if (substr($passdb, 0, 2) == 'U$') {
    // This may be an updated password from user_update_7000(). Such hashes
    // have 'U' added as the first character and need an extra md5().
    $stored_hash = substr($passdb, 1);
 ...
 *   TRUE or FALSE.
 */
function user_needs_new_hash($passdb) {
  // Check whether this was an updated password.
  if ((substr($passdb, 0, 3) != '$S$') || (strlen($passdb) != DRUPAL_HASH_LENGTH)) {
    return TRUE;
  }
  // Check whether the iteration count used differs from the standard number.
  return (_password_get_count_log2($passdb) != variable_get('password_count_log2', DRUPAL_HASH_COUNT));
}
