...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	if ($bulletinProvisoire == "non") {
		$pdf->MultiCell(210,5,"Attestation de Compétences\nSkills certificate",0,'C',0);
	}else{
		$pdf->MultiCell(210,5,"Attestation de Compétences\nSkills certificate",0,'C',0);
	}
	...
	$pdf->SetXY(0,$Y+=18);
	list($specification,$spec2)=preg_split('/-/',chercherSpecificationClasse($_POST["saisie_classe"]));
	$pdf->MultiCell(210,5,"$specification",0,'C',0);
	$pdf->SetXY(0,$Y+=5);
	$pdf->MultiCell(210,5,"$spec2",0,'C',0);
...
// ----------------------------------------------------
	...
	// code_ue,nom_ue,coef_ue,ects_ue,nom_ue_en,semestre,coef_certif
	$ectsTOTAL=0;
	...
	for($f=0;$f<count($recupUE);$f++) {
		...
		$coef_certif=$recupUE[$f][6];
		$dejapasse=0;
	
		$blocValide="Validé";

		if (($sem == '1') || ($sem == '2')) {
			$listeMatiere=recupMatiereUECoefCertifPositif($code_ue,$idClasse);
  		}else{
			$listeMatiere=recupMatiereUE2certifpositif($nom_ue,$idClasse);
		...
		if ($nbmatiere > 0){
			$pdf->MultiCell(190,8,trunchaine("$nom_ue\n$nom_ue_en",80),1,'L',1);
			$Y+=8;
		}
		...
		if ($affEtudeDeCas == "oui") {
			...
			if (!$verif) {
				$pdf->SetXY($X,$Y);
				$pdf->MultiCell(190,10,"Pas d'étude de cas / No case study",1,'C',0);
				$Y+=10;
			}else{
...
//		$pdf->MultiCell(15,5,"Coef. /\nweight",0,'C',0);   
//		$pdf->SetXY($X+=15,$Y+3);
		$pdf->MultiCell(150,10,"Matière / Subject",0,'C',0);   
		$pdf->SetXY($X+=150,$Y+3);
/*		if ($affExam == "oui") {
...
//		$pdf->MultiCell(13,5,"Credits",0,'C',0);   
		...
		for($i=0;$i<count($listeMatiere);$i++) {
			...
			$blocValide="Unité d'Enseignement validée\nValidated Study Unit";
...
//			$pdf->SetXY($X+=15,$Y);
			$pdf->MultiCell(150,$hauteurMatiere,trunchaine("$matiere / $matiereen",70),1,'C',0);   
			$pdf->SetXY($X+=150,$Y);
...
//			$pdf->SetXY($X+=10,$Y);
			...
			if ($note >= 10) $affichenote="Validé";
			if ($note < 10) { 
				$affichenote="Non validé"; 
				$blocValide="Unité d'Enseignement non validée\nNon validated Study Unit"; 
				$certifgeneral="non validé";  
			}
			$pdf->MultiCell(20,$hauteurMatiere,"$affichenote",1,'C',0);   
			$affichenote="";
			$pdf->SetXY($X+=10,$Y);
...
//			$pdf->MultiCell(13,$hauteurMatiere,"$ects",1,'C',0); 
	...
	if ($nbmatiere > 0){
		...
		$pdf->SetFillColor(220);
		$pdf->SetXY($X,$Y);	
		$pdf->MultiCell(190,15,"",1,'C',1);  
		$pdf->SetFont('Arial','B',10);
		$pdf->SetXY($X,$Y+2);	
		$pdf->MultiCell(100,4,"$nom_ue\n$nom_ue_en",0,'R',0); 
		$pdf->SetXY($X+105,$Y+2);	
...
//$pdf->MultiCell(190,15,"Moyenne Générale / General average  $moyenneGeneraleAff                                                            ",1,'C',1);
//$pdf->MultiCell(190,15,"Certification : $certifgeneral                                                            ",1,'C',1);
$certifgeneral="";
//$Y+=18;
...
$pdf->SetFont('Arial','',12);
$pdf->SetFillColor(255);
$pdf->RoundedRect($X=$Xorigine, $Y=$YSuite, $largeurBulle-5, 27, 3.5, 'DF');
$pdf->SetXY($X+2,$Y+1);
