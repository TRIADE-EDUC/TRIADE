function attente_serveur() {
	...
	this.afficher = function(str_message) {
		...
		eval("int_timeout_attente_serveur_" + this.id_local + " = setInterval(\"" + this.id_local + ".maj_position()\", 200)");
	}

	// Mettre a jour la position du message d'attente
	this.maj_position = function () {
		...
		obj_div_attente.style.top = (tab_pos_dim_div["scroll_height"] + 1) + 'px';
	...
	/*
	this.lire_dimensions = function () {
		// Try to get window dimensions

			if (navigator.appName == "Microsoft Internet Explorer")	{
				if(document.documentElement.clientHeight > 0) {
						eval('this.window_x = ' + this.getElementByIdPrefix + 'document.documentElement.clientWidth');
						eval('this.window_y = ' + this.getElementByIdPrefix + 'document.documentElement.clientHeight');
						eval('this.document_x = ' + this.getElementByIdPrefix + 'document.body.clientWidth');
						eval('this.document_y = ' + this.getElementByIdPrefix + 'document.body.clientHeight');
						this.scrollTop = document.documentElement.scrollTop;
						this.scrollLeft = document.documentElement.scrollLeft;
						// Get maximum y dimension
						if(this.window_y>this.document_y) {
							this.div_y=this.window_y;
							this.taille_min_y = this.document_y;
						} else {
							this.div_y=this.document_y;
							this.taille_min_y = this.window_y;
						}
						if(this.window_x>this.document_x) {
							this.div_x=this.window_x;
							this.taille_min_x = this.document_x;
						} else {
							this.div_x=this.document_x;
							this.taille_min_x = this.window_x;
						}
				} else {
						eval('this.window_x = ' + this.getElementByIdPrefix + 'document.body.clientWidth');
						eval('this.window_y = ' + this.getElementByIdPrefix + 'document.body.clientHeight');
						eval('this.document_x = ' + this.getElementByIdPrefix + 'document.body.clientWidth');
						eval('this.document_y = ' + this.getElementByIdPrefix + 'document.body.clientHeight');	
						this.scrollTop = document.body.scrollTop;
						this.scrollLeft = document.body.scrollLeft;
						this.div_y=this.document_y + this.scrollTop;
						this.taille_min_y = this.document_y;
						this.div_x=this.document_x + this.scrollLeft;
						this.taille_min_x = this.document_x;
				}
			} else {
