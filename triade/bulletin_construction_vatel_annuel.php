...
$classe_nom=$data[0][1];
$afficheECTSACQUIS=$_POST["afficheECTSACQUIS"];
...
//	$pdf->SetTitle("RELEVE DES NOTES - $nomEleve $prenomEleve");
	...
	$pdf->SetSubject(" $textTrimestre ");                // AJOUTER DETE DE DEBUT et FIN DE PERIODES
...
// 		$pdf->WriteHTML("<b>Classe </b>: $lib_classe - ".strtoupper($classe_nom));
	...
	$pdf->SetXY($Xmat,$Ymat);
	$pdf->MultiCell(195,7,'BULLETIN ANNUEL',1,'C',0);
	$pdf->SetFont('Arial','B',8);
	$pdf->SetXY($Xmat+117,$Ymat+1);
	$pdf->WriteHTML('du '.$dateDebutInfo.' au '.$dateFinInfo);
...
// deuxieme ligne
	...
	$Xmat1=$Xmat1+15;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(15,6,'Période 1',1,'C',0);

	$Xmat1=$Xmat1+15;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(15,6,'Période 2',1,'C',0);

	$Xmat1=$Xmat1+15;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(35,6,'Notes Période 2',1,'C',0);

	
//	$Xmat1=$Xmat1+15;
//	$pdf->SetXY($Xmat1,$Ymat);
//	$pdf->MultiCell(20,6,'Note sem 2',1,'C',0);

	$Xmat1=$Xmat1+35;
	$pdf->SetXY($Xmat1,$Ymat);
	$pdf->MultiCell(15,6,'ECTS',1,'C',0);
...
//	$liste_matiere=$_POST["listematiere"];
	...
	//		if ($noteperiode[0][0]!=0 && $noteperiode[0][0]!='') {$nb_mat+=1;}
		...
		//	$id_ue = $ecrit;
			...
			$pdf->Line($Xmat,$Ymat,$Xmat+195,$Ymat);
			$pdf->SetFont('Arial','',7);
			$pdf->SetXY($Xmat,$Ymat);
		...
		//	$pdf->MultiCell(40,6,' Moyenne min.:'.format_moyenne($periode[$id_ue]['min']).'- max:'.format_moyenne($periode[$id_ue]['max']),'T','C',0);
			$pdf->SetFont('Arial','',7);
				...
				$moyenne_periode2=$moyenne_periode2/$nbcoefperiode2 ;
                        ...
                        $pdf->SetXY($Xmat+115,$Ymat);
                        $pdf->MultiCell(15,6,''.format_moyenne($moyenne_periode1).'',1,'C',0);
			if (($moyenne_periode1 != "") && ($moyenne_periode1 >= 0 )) {
				...
				$nbcoef_periode_annuelle_ue++;
                        ...
                        $pdf->SetXY($Xmat+130,$Ymat);
                        $pdf->MultiCell(15,6,''.format_moyenne($moyenne_periode2).'',1,'C',0);
			...
			$pdf->SetXY($Xmat+145,$Ymat);
//			$pdf->MultiCell(15,6,''.format_moyenne($moyenne_periode_ue).'',1,'C',0);
			if ($moyenne_periode_ue>=0 && is_numeric($moyenne_periode_ue)) {
				$nb_mat_gen++;
...
//		$nom_ue='UE'.$ue[$nb_ue][1].' '.$ue[$nb_ue][2];
		...
		$pdf->MultiCell(195,6,''.strtoupper($nom_ue).'',1,'L',0);
...
//print($nb_ue.'coef_pa'.$notepartiel[0][6].' ;');
		...
		$affiche_moyenne_partiel_ects=$affiche_moyenne_partiel;
		...
		if ($moyenne_periode >=0 && is_numeric($moyenne_periode) && $nb_note_periode>0) { 
			...
			$pdf->MultiCell(15,$hauteurMatiere,$moyenne_periode,'R','C',0);
			...
			$pdf->SetFont('Arial','',7);
			$pdf->SetXY($Xmat+$largeurMat+40+20,$Ymat);
			$pdf->MultiCell(15,$hauteurMatiere,'','R','C',0);		
		}
		...
		for ($nb_note=0;$nb_note<count($noteperiode);$nb_note++) { 
			if ($noteperiode[$nb_note][0]>=0 && is_numeric($noteperiode[$nb_note][0]) && count($noteperiode)>0) {
				...
				if ($noteperiode[$nb_note][6]>1) {$aff_coef='('.$noteperiode[$nb_note][6].')';} else {$aff_coef='';}
				$liste_note = $liste_note.$noteperiode[$nb_note][0].$aff_coef." - ";
				$liste_note2 = $liste_note2.$noteperiode[$nb_note][0]." ";
				$nb_note_periode+=1;
				$som_coef_periode+=$noteperiode[$nb_note][6];
			}
		}
		...
		if ($moyenne_periode >=0 && is_numeric($moyenne_periode) && $nb_note_periode>0) { 
			$moyenne_periode=number_format($moyenne_periode,2,'.','');
			$pdf->SetFont('Arial','',7);
			$pdf->SetXY($Xmat+$largeurMat+40+20+15,$Ymat);
			$pdf->MultiCell(15,$hauteurMatiere,$moyenne_periode,'R','C',0);
		...
		if ($moyenne_periode >=0 && is_numeric($moyenne_periode) && $nb_note_periode>0) { 
			$moyenne_periode=number_format($moyenne_periode,2,'.','');
			$pdf->SetFont('Arial','',7);
//			$pdf->SetXY($Xmat+$largeurMat+40+20+15+15,$Ymat);
//			$pdf->MultiCell(15,$hauteurMatiere,$moyenne_periode,'R','C',0);
		} else {
			$pdf->SetFont('Arial','',7);
//			$pdf->SetXY($Xmat+$largeurMat+40+20+15+15,$Ymat);
//			$pdf->MultiCell(15,$hauteurMatiere,'','R','C',0);		
		...
		$pdf->SetFont('Arial','',6);
		$pdf->SetXY($Xmat+$largeurMat+40+20+15+15,$Ymat);
                $pdf->MultiCell(35,$hauteurMatiere,"$liste_note2",'R','L',0);
		$liste_note2="";

		$pdf->SetFont('Arial','',7);
		$pdf->SetXY($Xmat+$largeurMat+40+20+15+15+15+20,$Ymat);
		$ECTS=recupECTS($idMatiere,$idClasse,$_POST["saisie_trimestre"],$anneeScolaire);
                $pdf->MultiCell(15,$hauteurMatiere,"$ECTS",'R','C',0);

		if ($affiche_moyenne_partiel_ects >= 10) { $nbects+=$ECTS; } 
		unset($affiche_moyenne_partiel_ects);
...
//print_r ($periode);
			...
			$pdf->SetFont('Arial','B',7);
                        $pdf->MultiCell(70,6,' Moyenne  Annuelle /20 :','0','R',0);
                        // sam 
			$pdf->SetFont('Arial','B',7);
...
//			$pdf->WriteHTML('(min.:'.format_moyenne($moyenne_per[0]).' - max:'.format_moyenne(end($moyenne_per)).')</b>');
	...
	//		$pdf->MultiCell(15,6,''.format_moyenne($moyenne_periode_annuelle_ue).'',1,'C',0);
			...
			$pdf->SetFont('Arial','B',12);
                        $pdf->MultiCell(75,6,' Moyenne Générale Annuelle /20 :','1','R',1);
		...
		//	$pdf->WriteHTML('Rang  : '.((array_search($moyenne_annuelle_ue, $moyenne_par))+1).'/'.count($eleveT).'  - (min.:'.format_moyenne(end($moyenne_par)).' - max:'.format_moyenne($moyenne_par[0]).')');
			...
			if ($afficheECTSACQUIS == "oui") { 
				if ($moyenne_annuelle_ue >= 10) {
					$ectsresultat="ECTS ACQUIS : 60";
				}else{
					$ectsresultat="ECTS ACQUIS : $nbects";
				}
				$pdf->SetXY($Xmat+55+45-53+85,$Ymat);
                        	$pdf->SetFont('Arial','',12);
                        	$pdf->MultiCell(50,6,"$ectsresultat",0,'L',0);
			}
			...
			$Ymat=250; // pour decaller le bloc en bas de la page 230
			$moyenne_gen_partiel='';
			$somme_coef_gen_partiel=0;
//			$pdf->SetFont('Arial','',7);
...
// FIN Mise en place des moyenens general partiel et periode
	...
	$pdf->SetXY($Xmat,$Ymat);
//	$pdf->MultiCell(195,5,'',1,'',0);
	$pdf->SetXY($Xmat+2,$Ymat-15);
	$pdf->SetFont('Arial','',6);
	if ($nbheureabs >= 4) { 
		$nbabs+=$nbheureabs/4 ;
		$nbheureabs=$nbheureabs%4;
	} 
	$pdf->WriteHTML("<b>Nombre de 1/2 journées d'absences : </b>".$nbabs .' - <b>Nombre d\'heure d\'absences</b> : '.$nbheureabs.'  - <b>Nombre de retards :</b> '.$nbretard);
	//---------------------------------//
...
//$pdf->Line ($Xcom+2,$Ycom+20,$Xcom+175,$Ycom+20 );
//$pdf->Line ($Xcom+2,$Ycom+25,$Xcom+175,$Ycom+25 );
$pdf->SetXY($Xcom+2,$Ycom+5+5);
$pdf->SetFont('Arial','',8);
$pdf->MultiCell(175,3.5,$commentairedirection,'','','L',0); // commentaire de la direction (visa)
$pdf->SetDrawColor(0);
$pdf->SetXY($Xcom,$Ycom+$hauteurcom+1);
$pdf->SetFont('Arial','I',6);
$pdf->MultiCell(175,3.5,"NB: 60 ECTS sont validés sur l'année Académique si la moyenne générale >= 10/20 et le stage validé par la direction",'','C',0);
$pdf->SetXY($Xcom,$Ycom+$hauteurcom+3);
$pdf->MultiCell(175,3.5,"Document original - Aucun duplicata ne sera délivré",'','C',0);
