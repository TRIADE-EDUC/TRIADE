...
// +-------------------------------------------------+
// $Id: frame_facture.php,v 1.33 2019-05-28 15:12:23 btafforeau Exp $
...
function show_lig_bak() {
	...
	foreach($lig_afac as $key=>$value) {
		...
		$frame = str_replace('<!-- select_lgstat -->', lgstat::getLabelFromId($lig_afac[$key]['id_lgstat']), $frame);
		$frame = str_replace('!!id_lgstat!!', $lig_afac[$key]['id_lgstat'], $frame);
	...
	if ($max_lig_fac) {
		...
		foreach($lig_dfac as $key=>$value) {
			...
			$frame = str_replace('<!-- select_typ -->', $select_typ[1], $frame);
			$frame = str_replace('<!-- select_bud -->', $select_rub[1], $frame);
			$frame = str_replace('<!-- select_lgstat -->', lgstat::getLabelFromId($lig_dfac[$key]['id_lgstat']), $frame);
			$frame = str_replace('!!id_lgstat!!', $lig_dfac[$key]['id_lgstat'], $frame);
			$frame = str_replace('!!no!!', $index, $frame);
