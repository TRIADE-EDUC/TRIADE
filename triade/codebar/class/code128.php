...
//
// * Code C Working properly only on PHP4 or PHP5.0.0RC4+ due to bug :
...
class code128 extends BarCode {
	...
	public function code128($maxHeight,FColor $color1,FColor $color2,$res,$text,$textfont,$start='B') {
	...
	public function draw($im) {
		...
		for($i=0;$i<strlen($this->text);$i++) {
			if($this->currentCode=='C') {
				if(isset($this->text[$i+1]) && check_int($this->text[$i+1])) {
					$value_test = array_search($this->text[$i].$this->text[$i+1],$this->keys);
					$i++;
				}
				else {
					$this->DrawError($im,"With Code C, you must provide always pair of two integer.");
					$error_stop = true;
				}
			}
			else
