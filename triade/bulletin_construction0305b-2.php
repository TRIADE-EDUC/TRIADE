...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	$pdf->SetXY(80,$ycoor0-13);
	$pdf->MultiCell(100,3,"Année académique $anneeScolaire",0,'R',0);

	$pdf->SetFont('Arial','B',13);
	$pdf->SetFillColor(255);
	$pdf->SetXY(80,$ycoor0-5);
	$pdf->MultiCell(100,3,"RELEVE DE NOTES ET RESULTATS",0,'R',0);	

	$pdf->SetFont('Arial','B',12);
	$pdf->SetXY(80,$ycoor0+5);
	$pdf->MultiCell(100,3,"Etudiant(e) : $prenomEleve $nomEleve",0,'R',0);
	$pdf->SetXY(80,$ycoor0+15);
	$pdf->MultiCell(100,3,"Classe : $classe_nom_aff",0,'R',0);
	...
	$hauteurMatiere=7; // taille du cadre matiere
	...
	if ($nbmatiere >= 20) { $hauteurMatiere=7; }
...
//		if ($PP > 13) { $pdf->AddPage(); $Ymat=10; $PP=0; }
		$PP++;
		...
		if ($aff == 1) {
			...
			if (($coefMatiereUE > 0) && ($coefMatiereUE != "")) { 
				$moyenUE=$moyenUE/$coefMatiereUE;
				$moyenUE=number_format($moyenUE,2,'.','');
			}else{
				$moyenUE="";	
			}
			...
			$pdf->MultiCell($totalLargeur-5,4,"Moyenne du PE : $moyenUE",0,'R',1);  // MOYEN
			$pdf->SetFont('Arial','',7);		
			$Ymat=$Ymat + 4;

			if (($moyenUE >= 0) && (trim($moyenUE) != "")) {
				$moyenUEG+=$moyenUE;
				$coefUEG++;
			}

			/*----------------------- */
			if ((strtolower($titreTT) == "droit & sciences politiques") || (strtolower($titreTT) == "europe, relations internationales et géopolitique") || (strtolower($titreTT) == "economie & gestion")) {
				if (($moyenUE < 9) && ($moyenUE != "")) {
					foreach($tabRat as $key=>$value){
				                $pdf->SetFont('Arial','B',10);
				                list($XRat,$YRat)=preg_split('/:/',$value);
				                $pdf->SetXY(193,$YRat);
				                $pdf->MultiCell(5,3,"R",0,'C',0);
				        }
					$ajournee="oui";
				}
			}
			...
			$pdf->SetXY($Xmat,$Ymat);
			$pdf->MultiCell($totalLargeur-5,4,"PE - $titreEU1",0,'L',0);
			$titreTT=$titreEU1;
			$pdf->SetFont('Arial','',8);
			$pdf->SetTextColor(24,37,140);
			$Ymat+=4;
		...
		if ($dejafait == 0) {
			...
			$pdf->SetFont('Arial','B',9);
			$pdf->SetXY($Xmat,$Ymat);
			$pdf->MultiCell($totalLargeur-5,4,"PE $titreEU",0,'L',0);
			$titreTT=$titreEU;
			$pdf->SetFont('Arial','',8);
			$pdf->SetTextColor(24,37,140);
			$dejafait=1;
		...
		// Moyenne UE
		if (($noteaff1 >= 0) && ($noteaff1 != "")) { 
			$moyenUE+=$noteaff1*$coefMatiere;
			$coefMatiereUE+=$coefMatiere;
		}
		// Résultt (points)
		if ($visibleinfo == 1) {
...
/*if ((strtolower($titreEU) == "droit & sciences politiques") || (strtolower($titreEU) == "europe et relations internationales et géopolitiques") || (strtolower($titreEU) == "economie & gestion")) {
	if (($moyenUE < 9) && ($moyenUE != "")) {
		foreach($tabRat as $key=>$value){
                        $pdf->SetFont('Arial','B',10);
                        list($XRat,$YRat)=preg_split('/:/',$value);
...
$pdf->MultiCell($totalLargeur-5,5,"",0,'L',1);
...
if ($ajournee == "oui") {
	...
	$pdf->SetXY($Xmat-5,$ycoor0+1);
	$pdf->MultiCell($largeurMat+100,3,"en attente du rattrapage",0,'R',0);
}else{
	$pdf->MultiCell($largeurMat,3,"RESULTAT /20",0,'R',0);
}
...
if ($coefUEG > 0) {
	$moyenUEG=$moyenUEG/$coefUEG;
	unset($coefUEG);
	$noteMoyEleG=number_format($moyenUEG,2,',','');
	unset($moyenUEG);
}
