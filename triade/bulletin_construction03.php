...
$affichageMinMax=$_POST["affichageMinMax"];
config_param_ajout($affichageMinMax,"affichageMinMax");


$affichageTM1=$_POST["affichageTM1"];
config_param_ajout($affichageTM1,"affichageTM1");
...
if (count($valeur)) {
    ...
    if ($_POST["typetrisem"] == "trimestre") {
        if ($_POst["saisie_trimestre"] == "trimestre1" ) { $textTrimestre=LANGBULL22; $triabsconet="T1"; $affichageTM1="non";  }
...
for($j=$dep;$j<$nbEleveT;$j++) {  // premiere ligne de la creation PDF
	...
	for($i=0;$i<count($ordre);$i++) {
		...
		$pdf->MultiCell($largeurcom,$hauteurMatiere,'',1,'',0);
		...
		if ($idgroupe == "0") {
			...
			if ($triabsconet == 'T2') { $noteaffT1=moyenneEleveMatiere($idEleve,$ordre[$i][0],$dateDebutT1,$dateFinT1,$idprof); }
                        if ($triabsconet == 'T3') {
                               $noteaffT1=moyenneEleveMatiere($idEleve,$ordre[$i][0],$dateDebutT2,$dateFinT2,$idprof);
                               //$noteaffT1=moyenneEleveMatiere($idEleve,$ordre[$i][0],$dateDebutT1,$dateFinT1,$idprof);
                               $noteaffT2=moyenneEleveMatiere($idEleve,$ordre[$i][0],$dateDebutT1,$dateFinT1,$idprof);
                        }
			...
			if ($triabsconet == 'T2'){$noteaffT1=moyenneEleveMatiereGroupe($idEleve,$ordre[$i][0],$dateDebutT1,$dateFinT1,$idprof); }
                        if ($triabsconet == 'T3'){
                        	$noteaffT1=moyenneEleveMatiereGroupe($idEleve,$ordre[$i][0],$dateDebutT2,$dateFinT2,$idprof);
                        	$noteaffT2=moyenneEleveMatiereGroupe($idEleve,$ordre[$i][0],$dateDebutT1,$dateFinT1,$idprof);
		...
		$flecherouge="./image/commun/fleche_rouge.jpg";
		$flechevert="./image/commun/fleche_vert.jpg";
		if ($affichageTM1 == "oui") {
			if (($noteaff1 >= 0) && ($noteaffT1 >= 0) && (is_numeric($noteaffT1)) && ($noteaff1 > $noteaffT1)) $fleche=$flechevert ;
			if (($noteaff1 >= 0) && ($noteaffT1 >= 0) && (is_numeric($noteaffT1)) && ($noteaff1 < $noteaffT1)) $fleche=$flecherouge ;
			if (file_exists($fleche)) $pdf->Image($fleche,$XnotVal+10,$YnotVal-2,2,2);
			unset($fleche);
		}
		...
		if ($noteaffT1 >= 0) $coeffaffT1+=$coeffaff;
		if ($noteaffT2 >= 0) $coeffaffT2+=$coeffaff;
		...
		if ($affichageTM1 == "oui") {
			$pdf->SetXY($XmoyMatGVal-1,$YmoyMatGVal);
			$pdf->WriteHTML($noteaffT1);
			if ($triabsconet == 'T3') {
				$XmoyMatGVal+=10;
				$pdf->SetXY($XmoyMatGVal-1,$YmoyMatGVal);
				$XmoyMatGVal-=10;
				$pdf->WriteHTML($noteaffT2);
			}
		}


		$moyeMatGenaff=$moyeMatGen;
		if ($affichageMinMax == "oui" ) $pdf->WriteHTML($moyeMatGenaff);
	...
	if ($affichageMinMax != "oui" )  { $Xcom=$Xcom-32; }
	if ($affichageTM1 == "oui" )  { 
		$Xcom=$Xcom+10; 
		if ($triabsconet == 'T3') { $Xcom=$Xcom+10; } 
	}
	$pdf->SetFont('Arial','',$confPolice[0]);
	$pdf->SetXY($Xcom,$Ycom+0.2);
	$pdf->MultiCell($largeurcom,$confPolice[1],$commentaireeleve,'','','L',0);
	...
	if ($affichageTM1 == "oui") {
		if ( $noteaffT1 != "" ) {
			$noteMoyEleGT1Tempo = $noteaffT1 * $coeffaffT1;
			$noteMoyEleGT1=$noteMoyEleGT1 + $noteMoyEleGT1Tempo;
			$coefEleGT1=$coefEleGT1 + $coeffaffT1;
		}
		if ($triabsconet == 'T3') {
			$noteMoyEleGT2Tempo = $noteaffT2 * $coeffaffT2;
			$noteMoyEleGT2=$noteMoyEleGT2 + $noteMoyEleGT2Tempo;
			$coefEleGT2=$coefEleGT2 + $coeffaffT2;
		}
...
//	$pdf->MultiCell(11,8,'',1,'L',0);
}

if ($affichageTM1 == "oui") {
	$pdf->MultiCell(10,8,'',1,'L',0);
	if ($triabsconet == 'T3') {
		$pdf->SetXY($XMoyCL+10,$YMoyGE);
		$pdf->MultiCell(10,8,'',1,'L',0);		
	}
}
...
if ($affichageTM1 == "oui") {
	$pdf->SetFont('Arial','',9);
	$moyenEleveT1=moyGenEleve($noteMoyEleGT1,$coefEleGT1);
	$pdf->SetXY($XmoyElValue+13,$YmoyElGenValue);
	$pdf->WriteHTML("$moyenEleveT1");
	$flecherouge="./image/commun/fleche_rouge.jpg";
	$flechevert="./image/commun/fleche_vert.jpg";
	$moyenEleve=preg_replace('/,/','.',$moyenEleve);
	$moyenEleveT1=preg_replace('/,/','.',$moyenEleveT1);
        if (($moyenEleve >= 0) && ($moyenEleveT1 >= 0) && ($moyenEleve > $moyenEleveT1)) $fleche=$flechevert ;
        if (($moyenEleve >= 0) && ($moyenEleveT1 >= 0) && ($moyenEleve < $moyenEleveT1)) $fleche=$flecherouge ;
        if (file_exists($fleche)) $pdf->Image($fleche,$XmoyElValue+10,$YmoyElGenValue-1.5,2,2);
        unset($fleche);

	if ($triabsconet == 'T3') {
		$moyenEleveT2=moyGenEleve($noteMoyEleGT2,$coefEleGT2);
		$pdf->SetXY($XmoyElValue+13+10,$YmoyElGenValue);
	        $pdf->WriteHTML("$moyenEleveT2");
	}
}

$noteMoyEleG=0; // pour la moyenne de l'eleve general
$noteMoyEleGT1=0; // pour la moyenne de l'eleve general T-1
$coefEleG=0; // pour la moyenne de l'eleve general
$coefEleGT1=0; // pour la moyenne de l'eleve general
...
if ($affichageMinMax == "oui" ) $pdf->WriteHTML($moyenClasseGenaff);

$moyenClasseMinaff=$moyenClasseMin;
$pdf->SetXY($XmoyClasseMinValue,$YmoyClasseMinValue);
if ($affichageMinMax == "oui" ) $pdf->WriteHTML($moyenClasseMinaff);

$moyenClasseMaxaff=$moyenClasseMax;
$pdf->SetXY($XmoyClasseMaxValue,$YmoyClasseMaxValue);
if ($affichageMinMax == "oui" ) $pdf->WriteHTML($moyenClasseMaxaff);
// fin de la calcul de min et max
