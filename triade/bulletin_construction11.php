...
//	$pdf->WriteHTML($infoeleve2);
	...
	for($ik=0;$ik<count($dataadresse);$ik++) {
		...
		$civ1=$dataadresse[$ik][13];
		$civ2=$dataadresse[$ik][14];
		$civ1=civ($civ1);
		...
		$chaine="$civ1 ".trim(strtoupper($nomtuteur))." ".trim(ucwords(strtolower($prenomtuteur)));
		$pdf->SetXY($Xv11+6,39);
...
$pdf->SetFont('Arial','',8);
if (preg_match('/epicom.triade-educ.com/',$_SERVER["HTTP_REFERER"])) {
	//rien
}else{ 
...
if (preg_match('/epicom.triade-educ.com/',$_SERVER["HTTP_REFERER"])) {
	// rien
//	$pdf->WriteHTML("Avis du conseil de classe : ");
}else{
...
$bttexte=utf8_decode("Récupérer le fichier ZIP des bulletins");
